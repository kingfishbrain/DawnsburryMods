...
namespace DawnsburryMods.ClericRemastered
{
    public static class adjustSpells
    {
        public static void addSpells()
        {
            ...
            {
                return blessbane(true, IllustrationName.Bless, level);
            });
        ...
        private static CombatAction blessbane(bool isBless, IllustrationName illustration, int level)
        {
            return Spells.CreateModern(illustration, isBless ? "Bless" : "Bane", new Trait[4]
            {
                      ...
                      Trait.Divine,
                      Trait.Occult
            }, isBless ? "Blessings from beyond help your companions strike true." : "You fill the minds of your enemies with doubt.", isBless ? "For the rest of the encounter, you and your allies gain a +1 status bonus to attack rolls while within a 15-foot-radius emanation around you.\n\nOnce per turn, starting the turn after you cast bless, you can use a single action to increase the emanation's radius by 10 feet." : "You create a 10-foot-radius emanation around you. Enemies within it must succeed at a Will save or take a –1 status penalty to attack rolls as long as they are in the area.\n\nAn enemy who failed and then leaves the area and reenters later is automatically affected again. An enemy who enters the area for the first time rolls a Will save as they enter.\n\nOnce per turn, starting the turn after you cast bane, you can use a single action to increase the emanation's radius by 10 feet and force all not yet afflicted enemies in the area to attempt another saving throw.", Target.Self(), level, null).WithSoundEffect(isBless ? SfxName.Bless : SfxName.Fear).WithEffectOnSelf(async delegate (CombatAction action, Creature self)
            {
                CombatAction action2 = action;
                AuraAnimation auraAnimation = self.AnimationData.AddAuraAnimation(isBless ? IllustrationName.BlessCircle : IllustrationName.BaneCircle, isBless? 3f : 2f);
                QEffect qEffect = new QEffect(isBless ? "Bless" : "Bane", "[this condition has no description]", ExpirationCondition.Never, self, (Illustration)IllustrationName.None)
                {
                    ...
                    ProvideContextualAction = delegate (QEffect qfBless)
                    {
                        ...
                        return (!tag.Item2) ? new ActionPossibility(new CombatAction(qfBless3.Owner, illustration, isBless ? "Increase Bless radius" : "Increase Bane radius", new Trait[1] { Trait.Concentrate }, "Increase the radius of the " + (isBless ? "bless" : "bane") + " emanation by 10 feet.", Target.Self()).WithEffectOnSelf(delegate
                        {
